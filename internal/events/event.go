...
const (
	...
	// ModelCompletedV2 omits raw tool arguments from persisted model completions.
	...
	ModelListFailed    Type = "model.list.failed"
	ToolResult         Type = "tool.result"
	// ToolResultV2 persists tool exit status alongside bounded result metadata.
	ToolResultV2         Type = "tool.result.v2"
	ApprovalRequested    Type = "approval.requested"
	ApprovalGranted      Type = "approval.granted"
	ApprovalDenied       Type = "approval.denied"
	SessionConfigChanged Type = "session.config.changed"
	ApprovalModeChanged  Type = "approval.mode.changed"
